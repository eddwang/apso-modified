...
def open_script(settings, url, lineno, offset):
    if settings["EditorPath"]:
        cmd = '"{EditorPath}" {EditorArgs}'.format(**settings)
...
class RuntimeDialogBase(DialogBase):
    ...
    def create_button(self, name, command, pos, size,
        ...
        btn = self.create_control(name, "Button", pos, size, prop_names, prop_values)
...
class SyntaxErrorMessageDialog(RuntimeDialogBase):
    ...
    def _init(self):
        ...
        self.create_edit("edit_message",
                         ...
                         prop_values=(0, True, True, True, True))
        self.get("edit_message").getModel().Text = message
        if settings["EditorPath"] and ("{ROW}" in settings["EditorArgs"]):
            self.create_button("btn_ok", "ok",
...
class OrganizerDialog(NodeManager, RuntimeDialogBase):
    ...
    def __init__(self, ctx,
        ...
        self.menu = None
        self.tempdir = None
        self.settings = getApsoSettings(ctx)

    def execute(self, history=None):
        """ Show dialog with history represented as script in URI form."""
        self._create_ui()
        if self.settings["SaveLastScript"] or self.settings["SaveTreeState"]:
    ...
    def exec_menu(self, tree_node, node, rect=None):
        """ Shows dropdown menu. """
        if not self.menu:
            self._create_menu()
        menu = self.menu
        ...
        if rect:
            n = menu.execute(self.dialog.Peer, rect, 0)
        else:
    ...
    def exec_create_file(self, tree_node, node, filename=None):
        ...
        if name.strip():
            ...
            try:
                ...
                template = self.settings["DefaultHeader"]
    ...
    def exec_edit(self, tree_node, node):
        try:
            if node.uri.startswith(self.DOC_PROTOCOL):
                ...
                url = node.uri
            if self.settings["EditorPath"] and self._node_is_script(tree_node):
                lineno = self.getFuncLine(url, node.funcName)
            else:
                lineno = 0
            open_script(self.settings, url, lineno, 0)
        except Exception:
            raise ErrorAsMessage(traceback.format_exc())
    ...
    def exec_shell(self, tree_node, node):
        try:
            ...
            console(loc=loc,BACKGROUND=0x0,FOREGROUND=0xFFFFFF)
        except Exception:
    ...
    class KeyListener(ListenerBase, XKeyListener):
        def keyPressed(self, ev):
            if ev.KeyCode == 1280:
                self.act._key_pressed(ev.Modifiers)
            elif ev.KeyCode == 1305:
                self.act.button_pushed('menu')

        def keyReleased(self, ev):
            pass

    def _key_pressed(self, modifiers):
        node = self.tree_get_selected_node()
        if node:
            if self._node_is_script(node):
                self.dialog.endDialog(1)
            elif modifiers & MOD1 == MOD1:
    ...
    class MouseListener(ListenerBase, XMouseListener):
        ...
        def mouseReleased(self, ev):
            if ev.Buttons == 2:
                try:
                    if self.act.menu:
                        return
                    tree_node = self.act.tree_get_selected_node()
                    if tree_node:
                        node = self.act._node_get(tree_node)
                        X = ev.X + ev.Source.PosSize.X
                        Y = ev.Y + ev.Source.PosSize.Y
                        self.act.exec_menu(tree_node, node, Rectangle(X, Y, 0, 0))
                except ErrorAsMessage as e:
                    MessageDialog(self.act.ctx, self.act.dialog.getPeer(), type=ERRORBOX,
                                  title=RR.resolvestring('msg01'), message=str(e)).execute()
                except Exception as e:
                    print(e)
                    traceback.print_exc()
        ...
        def mousePressed(self, ev):
            if ev.ClickCount == 2 and ev.Buttons == 1:
                ...
                if self.act.menu:
                    self.act.menu = None
                tree_node = ev.Source.getClosestNodeForLocation(ev.X, ev.Y)
                ev.Source.select(tree_node)
    ...
    class TreeExpansionListener(ListenerBase, XTreeExpansionListener):
        ...
        def treeCollapsed(self, ev):
            if self.act.settings["UnloadChildsOnCollapse"]:
                node = ev.Node
                if node:
    ...
    def node_requested(self, tree_node):
        ...
        try:
            ...
            n = SyntaxErrorMessageDialog(
                            ...
                            message=str(e), settings=self.settings).execute()
            if n:
                if url.startswith(self.DOC_PROTOCOL):
                    try:
                        ...
                        url = self._create_tempfile(node)
                open_script(self.settings, url, e.lineno, e.offset)
            return
        except Exception as e:
            raise ErrorAsMessage(str(e))
        if node and not tree_node.getChildCount() and child_nodes:
            ...
            nodes = [(uno.fileUrlToSystemPath(child.getName()), child) for child in child_nodes]
    ...
    def get_tree_state(self):
        def _get_tree_state(treenode, dict_):
            if self.tree.isNodeExpanded(treenode):
                dict_[treenode.DisplayValue] = {}
                for n in range(treenode.ChildCount):
                    node = treenode.getChildAt(n)
                    if node == cursel:
                        dict_[treenode.DisplayValue] = node.DisplayValue
                    _get_tree_state(node, dict_[treenode.DisplayValue])
        nodes = {}
        root = self.tree.Model.DataModel.Root
        cursel = self.tree_get_selected_node()
        for idx in range(root.ChildCount):
            _get_tree_state(root.getChildAt(idx), nodes)
        return nodes
    ...
    def _set_history(self, history):
        ...
        def _set_tree_state(treenode, dict_):
            for key in dict_:
                for n in range(treenode.ChildCount):
                    childnode = treenode.getChildAt(n)
                    nodename = childnode.DisplayValue
                    if self._node_is_script(childnode) and self.settings["SaveLastScript"]:
                        if nodename == dict_:
                            self.tree_select_node(childnode)
                            break
                    elif nodename == key:
                        self._expand_node(childnode)
                        _set_tree_state(childnode, dict_[key])
                        break

        def _get_last_path(dict_):
            if not isinstance(dict_, dict):
                return [dict_]
            for key in dict_:
                res = _get_last_path(dict_[key])
                if not res:
                    continue
                else:
                    res.append(key)
        ...
        root = self.tree.Model.DataModel.Root
        if self.settings["SaveTreeState"]:
            _set_tree_state(root, history)
        else:
            lastpath = _get_last_path(history)
            treenode = root
            while lastpath:
                nodename = lastpath.pop()
                for n in range(treenode.ChildCount):
                    childnode = treenode.getChildAt(n)
                    if childnode.DisplayValue == nodename:
                        self._expand_node(childnode)
                        treenode = childnode
                        break
            self.tree_select_node(treenode)
    ...
    def tree_select_node(self, tree_node):
        """ Select tree node and ensure it is visible. """
        self.tree.select(tree_node)
        self.tree.makeNodeVisible(tree_node)
    ...
    def _collapse_node(self, tree_node):
        """ Request to expand node. """
        self.tree.collapseNode(tree_node)
    ...
    def _create_ui(self):
        ...
        if self.user_provider:
            add_node(RR.resolvestring('msg31'), self.user_provider, self.DISK_ICON)
        if self.share_provider:
            add_node(RR.resolvestring('msg32').format(PRODUCT), self.share_provider, self.DISK_ICON)
        if self.document_provider:
...
class PythonScriptOrganizer(unohelper.Base, XJobExecutor):
    ...
    def _store_history(self, tree_state):
        """ Store history. """
        self.__class__.History = tree_state
    ...
    def execute(self, show_user=True, show_share=True, show_doc=True, show_icon=True):
        ...
        if show_user:
            ...
            self.user_sp.uno_packages_sp = PythonScriptProvider(self.ctx, "user:uno_packages")
        if show_share:
            self.share_sp = PythonScriptProvider(self.ctx, "share")
            self.share_sp.uno_packages_sp = PythonScriptProvider(self.ctx, "share:uno_packages")
        if show_doc:
        ...
        dialog = OrganizerDialog(
                    ...
                    show_icon)
        result, mode, tree_state = dialog.execute(self.__class__.History)
        if result:
            self._store_history(tree_state)
    ...
    def trigger(self, arg):
        ...
        try:
            if arg in ("", "shortcut", "execute"):   # execute is kept for backward compatibility with existing user shortcuts
                loadResourceResolver(self.ctx)
                self.execute(True, True, True, True)
            elif arg.startswith('open::'):
                _, filename, lineno, offset = arg.split('::')
                settings = getApsoSettings(self.ctx)
                open_script(settings, filename, int(lineno), int(offset))
        except Exception:
...
class OptionsDialogHandler(unohelper.Base, XContainerWindowEventHandler):
    def __init__(self, ctx):
        self.ctx = ctx
        loadResourceResolver(self.ctx)
        self.CfgNode = "/apso.Settings/EditorKicker"
