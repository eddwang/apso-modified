...
import unohelper
import theconsole
from com.sun.star.awt import (Rectangle, WindowDescriptor, XActionListener, XWindowListener,
                              XMouseMotionListener, XTopWindowListener, FontDescriptor)
from com.sun.star.awt.WindowClass import SIMPLE
from com.sun.star.awt.ImagePosition import AboveCenter
from com.sun.star.awt.WindowAttribute import BORDER, MOVEABLE, SIZEABLE, CLOSEABLE, SHOW
from com.sun.star.awt.PosSize import (X as PS_X, Y as PS_Y, WIDTH as PS_WIDTH,
                                      HEIGHT as PS_HEIGHT, POSSIZE as PS_POSSIZE)
from com.sun.star.awt.SystemPointer import HSPLIT, VSPLIT

from apso_utils import msgbox, xray, console, getProductName, getConfigurationAccess
...
class MouseMotionListener(unohelper.Base, XMouseMotionListener):
    def __init__(self, topwindow, controls, orient):
        ...
        self._mouse_dragged = {'H': self._hori_dragged, 'V': self._vert_dragged}[orient]
    ...
    def _vert_dragged(self, e):
        ...
        self.controls['lb_code'].setPosSize(0, 0, 0, Y - (CODE_TOP + SPLITTER), PS_HEIGHT)
        self.controls['ft_output'].setPosSize(0, Y + SPLITTER + MARGIN, 0, 0, PS_Y)
        self.controls['ft_scope'].setPosSize(0, Y + SPLITTER + MARGIN, 0, 0, PS_Y)
        self.controls['lb_output'].setPosSize(0, footer_top, 0, height - (footer_top + MARGIN), PS_Y | PS_HEIGHT)
        self.controls['lb_scope'].setPosSize(0, footer_top, 0,  height - (footer_top + MARGIN), PS_Y | PS_HEIGHT)
        self.controls['lb_scope'].setPosSize(0, footer_top, 0,  height - (footer_top + MARGIN), PS_Y | PS_HEIGHT)
        self.controls['hsplitter'].setPosSize(0, Y, 0, height - Y, PS_Y | PS_HEIGHT)
    ...
    def _hori_dragged(self, e):
        ...
        self.controls['ft_output'].setPosSize(0, 0, X - 1.5*MARGIN, 0, PS_WIDTH)
        self.controls['lb_output'].setPosSize(0, 0, X - 1.5*MARGIN, 0, PS_WIDTH)
...
class Debugger(unohelper.Base, XActionListener, XWindowListener, XTopWindowListener):
    def __init__(self, uri, script, scriptprovider):
        ...
        self.create_ui()
        self.component = None
        self.getunotype = self.create("com.sun.star.reflection.CoreReflection").getType
        self.inspector = None
        try:
    ...
    def executedebug(self):
        if self.scripturi.endswith("document"):
            ...
            if self.component.hasLocation():
                self.currentcomponent = uno.fileUrlToSystemPath(self.component.Location)
            else:
    ...
    def create_ui(self):
        ...
        self.create_control('ft_code', 'FixedText',
                            (MARGIN + 1, CODE_TOP - LABEL_HEIGHT, WIDTH - 2*MARGIN - 1, LABEL_HEIGHT),
                            ('Label',), ('Label 1',))
        self.create_control('lb_code', 'ListBox', (MARGIN, CODE_TOP, WIDTH-2*MARGIN, SPLITTER_Y - (CODE_TOP + MARGIN)),
                            ('FontDescriptor', 'TextColor'), (fontdescriptor, CODE_COLOR), 'generic')
        # STACK VIEWER
        ...
        self.create_control('ft_output', 'FixedText',
                            (MARGIN + 1, SPLITTER_Y + SPLITTER + MARGIN, footer_width - 1, LABEL_HEIGHT),
                            ('Label',), (RR.resolvestring('lbl01'),))
        self.create_control('lb_output', 'ListBox', (MARGIN, footer_top, footer_width, FOOTER_HEIGHT),
                            ('AutoVScroll', 'BackgroundColor', 'FontDescriptor', 'HScroll', 'MultiLine'),
                            (True, 0xfdf6e3, fontdescriptor, True, True), 'generic')
        # SCOPE VIEWER
        self.create_control('ft_scope', 'FixedText',
                            (2*MARGIN + footer_width + SPLITTER + 1, SPLITTER_Y + SPLITTER + MARGIN, footer_width - 1, LABEL_HEIGHT),
                            ('Label',), (RR.resolvestring('lbl02'),))
        self.create_control('lb_scope', 'ListBox',
                            (2*MARGIN + footer_width + SPLITTER, footer_top, footer_width, FOOTER_HEIGHT),
                            ('BackgroundColor', 'FontDescriptor'), (0xe3e3ff, fontdescriptor), 'generic')
        # VERTICAL SPLITTER
        self.vsplitter = self.create_window(SIMPLE, "splitter",
                                            Rectangle(0, SPLITTER_Y, WIDTH, SPLITTER),
                                            SHOW, self.topwindow)
        ...
        self.vsplitter.addMouseMotionListener(MouseMotionListener(self.topwindow, self.controls, 'V'))

        # HORIZONTAL SPLITTER
        self.hsplitter = self.create_window(SIMPLE, "splitter",
                                            Rectangle((WIDTH - SPLITTER)/2, SPLITTER_Y, SPLITTER, HEIGHT - SPLITTER_Y),
                                            SHOW, self.topwindow)
        self.hsplitter.setBackground(SPLITTER_COLOR)
        pointer = self.create("com.sun.star.awt.Pointer")
        pointer.setType(HSPLIT)
        self.hsplitter.setPointer(pointer)
        self.hsplitter.addMouseMotionListener(MouseMotionListener(self.topwindow, self.controls, 'H'))

        # STARTING GUI
        self.controls.update(vsplitter=self.vsplitter, hsplitter=self.hsplitter)
    ...
    def getfont(self):
        ...
        fontdescriptor.Name = reader.FontName or "DejaVu Sans Mono"
        fontdescriptor.Height = reader.FontHeight
        fontdescriptor.Pitch = 1
        return fontdescriptor

    def create_window(self, wtype, service, rect, attributes=SHOW | BORDER, parent=None):
        descriptor = WindowDescriptor(wtype, service, parent, -1, rect, attributes)
    ...
    def create_control(self, name, type_, possize=None, prop_names=None, prop_values=None, action=''):
        ...
        if possize:
            ctrl.setPosSize(possize[0], possize[1], possize[2], possize[3], PS_POSSIZE)
        if action == 'generic':
    ...
    def actionPerformed(self, event):
        ...
        if source == self.controls['lb_code']:
            ...
            if value == UNKNOWN:
                ...
                try:
                    self.getunotype(varvalue)    # -> raise error if varvalue can be inspected
            ...
            try:
                if self.scripturi.endswith("document"):
                    ...
                    self.scriptprovider = PythonScriptProvider(self.ctx, self.component)
    ...
    def windowResized(self, event):
        try:
            ...
            code_height = footer_top - (CODE_TOP + SPLITTER + LABEL_HEIGHT + 2*MARGIN)
            if ps_base.Height > (CODE_TOP + footer_height + SPLITTER + LABEL_HEIGHT + 2*MARGIN):
                self.controls['ft_output'].setPosSize(0, footer_top - LABEL_HEIGHT, 0, 0, PS_Y)
                self.controls['lb_output'].setPosSize(0, footer_top, 0, 0, PS_Y)
                self.controls['lb_scope'].setPosSize(0, footer_top, 0, 0, PS_Y)
                self.controls['ft_scope'].setPosSize(0, footer_top - LABEL_HEIGHT, 0, 0, PS_Y)
                self.vsplitter.setPosSize(0, footer_top - (LABEL_HEIGHT + SPLITTER + MARGIN), 0, 0, PS_Y)
                self.hsplitter.setPosSize(0, footer_top - (LABEL_HEIGHT + SPLITTER + MARGIN), 0, 0, PS_Y)
                self.controls['lb_code'].setPosSize(0, 0, 0, code_height, PS_HEIGHT)
            if ps_base.Width > footer_width + SPLITTER + 2*MARGIN:
                self.controls['ft_output'].setPosSize(0, 0,
                                                      ps_base.Width - (footer_width + SPLITTER + 3*MARGIN - 1),
                                                      0, PS_WIDTH)
                self.controls['lb_output'].setPosSize(0, 0,
                                                      ps_base.Width - (footer_width + SPLITTER + 3*MARGIN),
                                                      0, PS_WIDTH)
                self.controls['ft_scope'].setPosSize(ps_base.Width - (footer_width + MARGIN - 1), 0, 0, 0, PS_X)
                self.controls['lb_scope'].setPosSize(ps_base.Width - (footer_width + MARGIN), 0, 0, 0, PS_X)
                self.vsplitter.setPosSize(0, 0, ps_base.Width, 0, PS_WIDTH)
                self.hsplitter.setPosSize(ps_base.Width - (footer_width + SPLITTER + 1.5*MARGIN), 0, 0, 0, PS_X)
                self.controls['lb_code'].setPosSize(0, 0, ps_base.Width - 2*MARGIN, 0, PS_WIDTH)
        except Exception:
            traceback.print_exc()
...
class Apsodb(bdb.Bdb, threading.Thread):
    ...
    def __init__(self, dialog):
        ...
        self.toprealfilename, self.topdisplayname = self.realfilename(self.topfilename)
    ...
    def trace_dispatch(self, frame, event, arg):
        ...
        if event == 'line':
            ...
            return self.trace_dispatch
        self.dialog.writeoutput(('bdb.Bdb.dispatch: unknown debugging event:', repr(event)))
        return self.trace_dispatch
    ...
    def user_line(self, frame):
        ...
        if self.dialog.ui_output.Model.TextColor == self.EXCEPTION:
            line = linecache.getline(self.currentfilename, frame.f_lineno).strip()
            ...
            self.dialog.ui_output.selectItemPos(self.dialog.ui_output.ItemCount-1, True)
        self.setscope(frame)
        self.catch_exc = 0
    ...
    def user_return(self, frame, retval):
        ...
        if len(self.stack) > 1:
            ...
            self.setscope(frame)
            name = frame.f_code.co_name
            self.dialog.writeoutput('[{}] returns: {}'.format(name, retval), textcolor=self.RETURN)
        # self.wait_for_action(frame)
        ...
        if (frame.f_code.co_filename == runningfile and frame.f_code.co_name == 'run'):
            ...
            for innerframe, filename, lineno, name, lines, index in inspect.getinnerframes(exc_traceback):
                ...
                if addtostack:
                    if not name:
                        name = '???'
                    func_argvalues = '{}{}'.format(name, inspect.formatargvalues(*inspect.getargvalues(innerframe)))
                    self.stack.append(func_argvalues)
            ...
            msgbox('{}: {}'.format(exc_type_, exc_value), win=self.dialog.topwindow)
            self.catch_exc = 1
            sys.settrace(None)
            # post mortem actions
            self.wait_for_action(innerframe)
        else:
            self.dialog.writeoutput('{}: {}'.format(exc_type_, exc_value), self.EXCEPTION)
            # self.catch_exc = 0
    ...
    def do_jump(self, frame):
        ...
        if currentline == self.lineno:
            ...
            msgbox(RR.resolvestring('msg37'), boxtype='warning', win=self.dialog.topwindow)
            self.dialog.gotoline(currentline-1)
            self.wait_for_action(frame)
        elif currentline > self.lineno:
            msgbox(RR.resolvestring('msg38'), boxtype='warning', win=self.dialog.topwindow)
            self.dialog.gotoline(currentline-1)
            self.wait_for_action(frame)
        elif not self.checkline(self.lineno):
            msgbox(RR.resolvestring('msg39'), boxtype='warning', win=self.dialog.topwindow)
            self.dialog.gotoline(currentline-1)
            self.wait_for_action(frame)
        else:
    ...
    def do_console(self, frame):
        try:
            ...
            console(WIDTH=400, loc=loc,BACKGROUND=0x0,FOREGROUND=0xFFFFFF, parent=self.dialog.topwindow)
            self.wait_for_action(frame)
        except Exception:
    ...
    def updatesource(self, filename, displayname):
        ...
        if filename in self.sources:
            ...
            for n, line in enumerate(_source, 1):
                try:
                    source.append('{:>{nalign}}.  {}'.format(n, line.decode('utf-8').encode(ENCODING), nalign=nalign))
                except Exception:
                    source.append('{:>{nalign}}.  {}'.format(n, line, nalign=nalign))
    ...
    def setscope(self, frame):
        ...
        nbarg = 0
        # local namespace
        varnames = sorted(frame.f_code.co_varnames[nbarg:], key=lambda s: s.lower())
        varvalues = []
        # # TODO: how to get some classes infos
        for var in varnames:
            varvalues.append('{} : {}'.format(var, self.repr_(f_locals.get(var, self.EmptyVar()))))
